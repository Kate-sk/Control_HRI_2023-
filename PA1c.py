...
##Compute the height map and the gradient along x and y -- see A1b
...
god = pygame.Rect(*screenHaptics.get_rect().center, 0, 0).inflate(48, 48)
god.center = (0, 0)
collision = False
leftwall = False
topwall = False
...
while run:
    ...
    ##Step 3 Virtual shapes     -- see PA1b
    
    ##Step 4 Virtual wall -- PA1c
    kc = 0.1
    fc = np.zeros(2)
    if haptic.colliderect(wall):
        if topwall:
            fc[0] = (haptic.right - wall.left) * kc
        elif leftwall:
            fc[1] = (haptic.bottom - wall.top) * kc
        fe += fc
    
    ##Step 5 Bonus: Friction
    mu = 0.005 # friction coefficient
    if haptic.colliderect(wall):
        if topwall:
            if xhold[1] > xh[1]:
                fc[1] = -(haptic.right - wall.left) * mu
            else:
                fc[1] = (haptic.right - wall.left) * mu
        elif leftwall:
            if xhold[0] > xh[0]:
                fc[0] = -(haptic.bottom - wall.top) * mu
            else:
                fc[0] = (haptic.bottom - wall.top) * mu
        fe += fc
    ...
    if haptic.colliderect(wall):
        if collision:                  #Check if a collision was already happening
            if leftwall:
                god.centerx = xh[0]
                god.bottom = wall.top    
                if (xh[1]-wall.top)<24:                         # implement compression of proxy
                    god.inflate_ip(0, -np.abs(xh[1]-wall.top))
                else:
                    god.inflate_ip(0, -24)
                ...
                god.centery = xh[1]
                god.right = wall.left
                if (xh[0]-wall.left)<24:
                    god.inflate_ip(-np.abs(xh[0]-wall.left), 0)
                else:
                    god.inflate_ip(-24, 0)
        elif (haptic.centerx - wall.left) > (haptic.centery - wall.top):
            leftwall = True              # Set the collision to a collision against the left wall
            god.centerx = xh[0]
            god.bottom = wall.top  
            if (xh[1]-wall.top)<24:
                god.inflate_ip(0, -(xh[1]-wall.top))
            else:
                god.inflate_ip(0, -24)
        else:
            topwall = True               # Set the collision to collision against the top wall 
            god.centery = xh[1]
            god.right = wall.left
            if (xh[0]-wall.left)<24:
                god.inflate_ip(-np.abs(xh[0]-wall.left), 0)
        ...
        pygame.draw.rect(screenVR, colorHaptic, god, border_radius=8) # show the god rectangle in case of collision
        collision = True      # set to "collision happening"
    else:
        pygame.draw.rect(screenVR, colorHaptic, haptic, border_radius=8) # show the haptic rectangle in case of no collision
        collision = False     # reset to "no collision happening"
        leftwall = False
        topwall = False
        god.update(0,0,48,48)

        
    
    pygame.draw.rect(screenVR, cDarkblue, wall) # Visualize wall
